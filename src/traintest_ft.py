...
def train(audio_model, train_loader, test_loader, args):
    ...
    while epoch < args.n_epochs + 1:
        ...
        for i, (a_input, v_input, labels) in enumerate(train_loader):
            ...
            data_load_time.update(time.time() - data_load_start)
            print(f'Batch {i}: Data loading took {data_load_time.val:.4f} seconds')
            ...
            if print_step and global_step != 0:
                print(
                    "Epoch: [{0}][{1}/{2}]\t"
                    "Per Batch Load Time {data_load_time.avg:.5f}\t"
                    "Per Batch Total Time {batch_time.avg:.5f}\t"
